...
They are Arista cEOS boxes with their own rendered config artifact
...
per VPN** — a customer is one routing domain across its sites — and links it as
the VPN's `customer_asn`. A site may still set `bgp_peer_asn` to override it,
which is how the **isp** dataset peers with pre-agreed customer AS numbers; when
every eBGP site overrides, no pool ASN is consumed.
