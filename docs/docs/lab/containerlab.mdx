...
- Docker with access to pull:
  - `ceos:latest` (requires an Arista account and local image import — see
    [the containerlab cEOS guide](https://containerlab.dev/manual/kinds/ceos/))
...
2. **cEOS** — the image is not on any public registry. Download the `.tar.xz`
   from the [Arista software download page](https://www.arista.com/en/support/software-download)
   and import it:
