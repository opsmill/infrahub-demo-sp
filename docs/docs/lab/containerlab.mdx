...
shared /31 addressing rather than hardcoded — so the lab tracks whatever the
active dataset's backbone defines.
...
On the **isp** dataset the same command brings up just `pe-lon-arista` (ceos)
and `pe-par-nokia-lab` (srl), plus that dataset's CE nodes.
